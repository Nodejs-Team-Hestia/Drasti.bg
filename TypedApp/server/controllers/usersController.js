...
function getAllUsers(req, res, next) {
    ...
    User.find({}).where({ username: new RegExp(username, "i") }).where({ firstName: new RegExp(firstName, "i") }).where({ lastName: new RegExp(lastName, "i") }).skip(DEFAULT_PAGE_SIZE * (page - 1)).limit(DEFAULT_PAGE_SIZE).select('_id username firstName lastName imageUrl').exec(function (error, users) {
        if (error) {
            res.status(400);
            res.send(error);
        } else {
